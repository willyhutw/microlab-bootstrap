...
main() {
  ...
  # cilium
  helm upgrade --install -n kube-system cilium cilium/cilium --create-namespace -f ./helm-values/cilium.yml --version 1.17.2 --wait
  ...
  # cert-manager
  helm upgrade --install -n cert-manager cert-manager jetstack/cert-manager --create-namespace --set installCRDs=true --version 1.17.1 --wait
  ...
  # cilium-hubble
  helm upgrade --install -n kube-system cilium cilium/cilium --reuse-values -f ./helm-values/cilium-hubble.yml --version 1.17.2 --wait
  kubectl apply -f resources/hubble

  # argocd
  helm upgrade --install -n argocd argocd argo/argo-cd --create-namespace -f ./helm-values/argocd.yml --version 7.8.13 --wait
